...
async function fetchOccupancy(mooringIds: string[]): Promise<Record<string, number>> {
 ...
 * useProviderEarnings
 *
 * Fetches and aggregates all booking/earnings data for the logged-in provider.
 * Uses two Supabase queries (one for bookings + moorings, one for occupancy)
 * and computes everything else client-side.
 *
...
export function useProviderEarnings(filterPaid = false) {
    ...
    return useQuery<ProviderEarningsSummary>({
        ...
        queryFn: async () => {
            ...
            // 1. Fetch all provider bookings (exclude cancelled), joined with moorings name
            let query = supabase
                .from('bookings')
                .select(`
        ...
        `)
            ...
            // 2. Group bookings by mooring_id
            ...
            // 3. Fetch occupancy data for all distinct moorings (single IN query)
            const mooringIds = Array.from(mooringMap.keys());
            const occupancy = await fetchOccupancy(mooringIds);
            const DAYS_IN_YEAR = 365;

            // 4. Build per-mooring earnings stats
            ...
            // 5. Sort by net earnings descending — first item = best mooring
            byMooring.sort((a, b) => b.netEarnings - a.netEarnings);

            // 6. Aggregate overall totals
