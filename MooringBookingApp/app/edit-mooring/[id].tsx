...
export default function EditMooringScreen() {
  ...
  React.useEffect(() => {
    if (!id) return;
    const fetchMooring = async () => {
      try {
        const { data, error } = await supabase.from('moorings').select('*').eq('id', id).single();
        if (error) throw error;
        if (data) {
          setName(data.name || '');
          setLocation(data.location || '');
          setCountry(data.country || 'Croatia');
          setCountryFlag(data.country_flag || '🇭🇷');
          setDescription(data.description || '');
          setLat(data.lat ? data.lat.toString() : '');
          setLng(data.lng ? data.lng.toString() : '');
          setPricePerNight(data.price_per_night ? data.price_per_night.toString() : '');
          setDiscountPercent(data.discount_percent ? data.discount_percent.toString() : '0');
          setMaxBoatLength(data.max_boat_length ? data.max_boat_length.toString() : '');
          setMaxDraft(data.max_draft ? data.max_draft.toString() : '');
          setMooringUnits(data.mooring_units ? data.mooring_units.toString() : '1');
          setWindProtection(data.wind_protection || 'good');
          setSelectedAmenities(data.amenities || []);
          setIsLastMinute(data.is_last_minute || false);
          setIsNow4Today(data.is_now4today || false);
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const { data, error } = await supabase.from('moorings').update({
        name: name.trim(),
      ...
      Alert.alert('Error', err.message || 'Failed to update mooring');
    }
    setSaving(false);
  };
  ...
  return (
    <SafeAreaView style={s.container}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>⚓ Edit Mooring</Text>
        <View style={{ width: 38 }} />
      </View>
